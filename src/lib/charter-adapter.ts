...
import type { Charter, Trip, Captain, Pickup, Policies, FishingType, Tier } from "@/dummy/charter";
...
function convertTrip(backendTrip: BackendTrip): Trip {
  return {
    ...
    duration: `${backendTrip.durationHours} hour${backendTrip.durationHours !== 1 ? 's' : ''}`,
...
function convertCaptain(backendCaptain: {
  ...
  return {
    name: backendCaptain.displayName || `${backendCaptain.firstName} ${backendCaptain.lastName}`,
 ...
 */
function convertPickup(backendPickup: {
  available: boolean;
  fee: number | null;
  notes: string | null;
 ...
 */
function convertPolicies(backendPolicies: {
 ...
 */
export function convertBackendCharterToFrontend(backendCharter: BackendCharter): Charter {
  // Extract images from media
  const images = backendCharter.media
  ...
  const coordinates = 
    backendCharter.latitude && backendCharter.longitude
      ? { lat: Number(backendCharter.latitude), lng: Number(backendCharter.longitude) }
      : undefined;
 ...
 */
export function convertBackendChartersToFrontend(backendCharters: BackendCharter[]): Charter[] {
